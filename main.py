...
# Checkpoints
parser.add_argument('--logs-dir',default='log_test', type=str, metavar='PATH',
...
def train_model(train_loader, model, optimizer, criterion, epoch):
	...
	outputs_a, outputs_b = torch.squeeze(outputs_a), torch.squeeze(outputs_b)
        ...
        loss = criterion(outputs_a, outputs_b, num_person, num_same)
...
def main():
    ...
    criterion = TripletLoss(margin=4).cuda()
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        print()
        loss = train_model(train_loader, new_model, optimizer, criterion, epoch)
